...
- ADR-0001 selects Electron as a narrow adapter around the shared
  React/TypeScript workbench and Core contracts. Do not wrap the current
  monolithic HTML, add Nativefier, expose generic IPC, or claim desktop support
  until the engine, extraction, and Windows/macOS target-host gates pass.
...
- Architecture decision: share one React/TypeScript workbench across the browser
  fallback and a narrow Electron Windows/macOS shell. ADR-0001 records the
  repository runtime inventory and the blocking engine, security, packaging,
  update, and target-host gates. Do not add Nativefier or duplicate Core in Rust.
