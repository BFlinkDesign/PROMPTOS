...
The locked, mechanized canonical for this section:
[locked/01-VERIFY-LOOP.md](../locked/01-VERIFY-LOOP.md).
...
Canonical cross-session block: [token-economy](../prompts/token-economy.md).
...
2. A **PROMPTOS repo layout** — done: this repo's `prompts/` (paste-in blocks),
   `patterns/` + `playbooks/` (architecture), `locked/` (invariants), and
   `behaviors/` (the parameterized contract) are that layout. Instantiated
   copies and retro rollups live per-project.
