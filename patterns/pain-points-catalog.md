...
| Same bug returns next session | No ledger / no promotion | [agent-memory](agent-memory.md) + [miss-promotion](../prompts/miss-promotion.md) |
...
| Git on network share is slow | Repo on SMB | Local canonical clone (not a network share) |
...
| Research returns the same three famous answers | Training-data default bias | [deep-procurement](../prompts/deep-procurement.md) — hidden-gem + disconfirmation quotas |
| The game-changing release never surfaces | No standing radar loop | [opportunity-scout](../prompts/opportunity-scout.md) on a schedule |
| Agent answers, then goes silent | Reactive-only defaults | Scout cadence + briefs that end in a smallest experiment |

## Add new entries

When a failure is not listed: one table row here (via PR to PROMPTOS) + one
line in the project's `ERRORS.md` (immediate, no waiting for PR). Distillation
process: [doctrine-distiller](../prompts/doctrine-distiller.md).
